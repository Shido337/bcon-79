...
const productWithAverageRating = products.map(getProductWithAverageRating);

function getProductWithAverageRating(product, index, array) {
  const totalRating = product.reviews
    .map(review => review.rating)
    .reduce((acc, ratingValue) => (acc += ratingValue), 0);

  const averageRating =
    Math.round((totalRating / product.reviews.length) * 100) / 100;
...
// console.table(productWithAverageRating);
const markup = productWithAverageRating.map(createProductsMarkup).join("");
// console.log("🚀 ~ markup:", markup);

function createProductsMarkup(product) {
  return `
    <li class="card col-4" id="${product.id}">
      <img src="${product.thumbnail}" class="card-img-top" alt="${product.title}" />
      <div class="card-body">
        <h6 class="card-card-title">
         ${product.title}
        </h6>
        <div class="row justify-content-between">
          <small class="col text-fw-bold">${product.averageRating}</small>
          <small class="col text-fw-bold">${product.price} $</small>
        </div>
      </div>
    </li>
    `;
...
productsList.innerHTML = productWithAverageRating
  .map(createProductsMarkup)
  .join("");
