...
// outerShowTitle(); // Який this undefined ❌ Error -> undefined.title
...
const chopShop = {
  ...
  calcTotalPrice(stoneName) {
    const findStone = this.stones.find((stone, index, array) => {
      return stone.name === stoneName;
      // stone.name === stoneName;
    });

    //? Паттерн раннє поверненя
    //? Якщо отримали негативний результат: перериваємо виконання функції
    if (!findStone) {
      console.log(`Stone: ${stoneName} is out of stock`);
...
console.log(chopShop.calcTotalPrice("Schebin")); // 0
